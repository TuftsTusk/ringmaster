...
function examineHistory(cursor_session, repopulate) {
    var history = cursor_session.history;
    var listing_ids = [];
    for (var doc_id = history.pop(); doc_id != undefined && listing_ids.length < Cosnts.MAX_LISTING_RESULTS; doc_id = history.pop()) {
        listing_ids.push(doc_id);
    }

    if (repopulate) {
        for (var i=0; i<listing_ids.length; i++) {
            history.push(listing_ids[i]);
        }
    }
    
    var params = cursor_session.params;
    params['_id'] = {
        $in: listing_ids
    }
...
function extractPreviousListings(cursor_session) {
    return examineHistory(cursor_session, false);
}

function extractCurrentListings(cursor_session) {
    return examineHistory(cursor_session, true);
}

function storeListingHistory(cursor_session, listings) {
    for (var l in listings) {
        cursor_session.history.push(l);
    }
}
...
function getListings(request, callback) {
    ...
    if ('search_id' in query) {
        var c_session = getCursorSession(query.search_id);
        if (!c_session) {
            return callback('The search session was not found.');
        } else {
            var direction = 'dir' in query ? query.dir : false;
            if (direction == 'prev') {
                listings = extractPreviousListings(c_session);
            } else if (direction == 'next') {
                listings = extractNextListings(c_session);
            } else {
                listings = extractCurrentListings(c_session);
            }
        }
    } else {
        var params = {
    ...
    return callback(false, listings);
}

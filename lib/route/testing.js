...
function hasUrlPermission(url, method, user_roles) {
    return hasUrlPermission(url, method, user_roles, ENV === DEV || ENV === STG);
}

function hasUrlPermission(url, method, user_roles, incl_dev) {
    console.log("Does "+user_roles.toString(16)+" have permission for "+url+" "+method);
    wl = WHITELIST;
    if (incl_dev)
        wl = wl.concat(DEV_WHITELIST);
...
function hasUrlPermission(url, method, user_roles, whitelist) {

                ...
                if (password != request.body.confirmpass) {
                    ...
                    Unconf_User.findOne({email: email}, function(err, user) {
                        ...
                        newuser.save(function(err) {
                            ...
                            if (!err) {
                              return emailUtils.sendConfirmationEmail(request.headers.host, newuser.email, newuser.confirmationKey, newuser._id,
                                function(error){
                                  if (error){
...
exports.postMeLogin = function(request, response) {
    ...
    User.findOne({email:request.body.email}, function(err, user) {
        if (err) {
            ...
            response.status(404).send(Common.error('USER_NOT_FOUND_FAILURE', 'The user with that email was not found'));

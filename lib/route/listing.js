...
function makeNewListingFromPost(body, user_id, callback) {
    ...
    if ('type' in body) {
        if (body.type === Listings.MISC) {
            ...
            updateMiscListingLatLng(miscListing, function(result) {
                if (result) {
                    return callback({
                        type: body.type,
                        listing: miscListing
...
exports.postListingById = function(request, response) {
    ...
    Listings.Listing.findOne({_id: listing_id}, function(err, listing) {
        if (err || !listing) {
            return response.sendStatus(404);
        } else {
            if (listing.user_id != request.session.who.id){
               return response.status(403).send(Common.error(
                    'EDIT_POST_NOT_PERMITTED',
                    'Listing seller id does not match editor attempt'
                ));
            }

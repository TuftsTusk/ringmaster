...
function makeNewListingFromPost(body, user_id, callback) {
    ...
    if ('type' in body) {
        if (body.type === Listings.MISC) {
        ...
        } else if (body.type === Listings.BOOK) {
        ...
        } else if (body.type === Listings.FURNITURE) {
          var furnitureListing = new Listings.FurnitureListing;
          for (var path in Listings._FurnitureSchema.paths) {
              var first_period = path.indexOf('.');
              var real_path = path;
              if (first_period != -1) {
                  real_path = path.substring(0, first_period);
          ...
          Utils.copyObjectData(furnitureListing, body, paths);
          return callback({
              type: body.type,
...
exports.postListingById = function(request, response) {
    ...
    Listings.Listing.findOne({_id: listing_id, user_id: request.session.login.who.id}, function(err, listing) {
        if (err || !listing) {

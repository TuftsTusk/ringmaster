    ...
    listingHandler.getListingByID(listing_id, function(err, listing) {
        if (err || !listing) {
            ...
            for (var key in request.body) {
                if (!(key in listingHandler.getListingSchema(listing.type).tree)) {
                    return response.status(400).send(Common.error(
...
exports.getListing = function(request, response) {
    response.set('Content-Type', 'application/json');
    if ('search_query' in request.query) {
        var listing_type = 'type' in request.query ? request.query.type : null;
        return listingHandler.getListingsBySearch(request.query.search_query, listing_type, function(err, listings) {
            if (err) {
        ...
        var listing_type = 'type' in request.query ? request.query.type : null;
        var pageNum = 'pageNum' in request.query ? request.query.pageNum : 1;
        return listingHandler.getListings(listing_type, pageNum, function(err, listings) {
            if (err) {
                return response.status(400).send(Common.error('Invalid listing request'));
            } else {
                var secure_wrapped_listings = listings.map(function(elem) {
        ...
        if (err) {
            ...
            return User.findOne(params)
                .exec(function(err, user){
                    if (!user || !user.email){
                        email.contactSeller(request.headers.referer, request.session.login.who.email,
                            'support@tuskmarketplace.com', 'Critical Error: User tried to contact an invalid user (user doesnt exist). Please delete this listing that has no user/ The original message to the seller was: ' + message,
                            listing._id,function(){});
                        return response.status(400).send(Common.error('We are unable to reach this seller. Please try again later.', err));
                    }
                    email.contactSeller(request.headers.referer, request.session.login.who.email, user.email, message, listing._id, function(error){
                      if (error){
                        return response.status(400).send(Common.error('We are unable to reach this seller. Please try again later.', error));
                      }
                      else {
                        return response.sendStatus(204);
                      }

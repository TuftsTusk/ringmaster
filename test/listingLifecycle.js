...
var postMiscPostById = function(cookie, id, data, callback) {
    request(app)
        .post('/listing/'+id)
        .send(data)
        .set('Cookie', cookie)
...
describe('Listing lifecycle', function() {
    ...
    it('Successfully add, edit, and remove a single listing', function(done) {
        var email = 'some.jerk@tufts.edu';
        var pass = 'foo';
        account.logInToAccount(email, pass, function(err, res) {
            if (err) done(err);
            ...
            makeMiscPost(cookie, {
                ...
                var data = {
                    title: 'New Title',
                    description: 'descriptiooooooooon hereeeeeeeee',
                    price: 1,
                    integrity: {
                        visible: false
                    }
                };
                postMiscPostById(cookie, rsc_id, data, function(err, res) {
                    if (err) done(err);
                    expect(res.status).to.equal(204);
                    deletePostFromId(rsc_id, function(err, res) {
                        SessionStorage.destroy(sid);
